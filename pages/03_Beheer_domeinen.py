...
# Detecteer DomainValues-laag en optionele FieldConfig-tabel
...
if "tables" in svc and svc["tables"]:
    # Kijk of er een FieldConfig-tabel bestaat met de verwachte kolommen
    for t in svc["tables"]:
...
# 1) Kies veld (alle unieke field_name uit DomainValues)
...
if fieldcfg_url:
    ...
    with c3:
        max_len = st.number_input("Max. lengte", value=int(existing_fc.get("max_len") or 0), step=1)
    with c4:
        required = st.checkbox("Verplicht", value=bool(existing_fc and existing_fc.get("required")))
else:
    st.info("Geen FieldConfig-tabel gedetecteerd — alleen domeinwaarden zijn te beheren.")

# 3) Domeinwaarden voor het gekozen veld
...
if st.button("Opslaan wijzigingen"):
    try:
        # Huidige records voor dit veld (OBJECTID nodig)
        ...
        cur_by_val = {str(r.get("domain_value")): r for r in cur_rows if r.get("domain_value") is not None}

        new_vals = set(edited["domain_value"].astype(str))
        cur_vals = set(cur_by_val.keys())
        ...
        for _, r in edited.iterrows():
            ...
            if dv in cur_by_val:
                attrs["OBJECTID"] = cur_by_val[dv].get("OBJECTID")
                to_upd.append({"attributes": attrs})
            else:
                to_add.append({"attributes": attrs})

        # Delete (records die niet meer in edited staan)
        to_del_vals = cur_vals - new_vals
        if to_del_vals:
            # veilige WHERE opbouwen
            clauses = [f"(field_name='{field_name.replace(\"'\",\"''\")}' AND domain_value='{v.replace(\"'\",\"''\")}')" for v in to_del_vals]
        ...
        # FieldConfig wegschrijven
        if fieldcfg_url:
            if existing_fc:
                upd = dict(existing_fc)
                upd["is_dropdown"] = 1 if is_dropdown else 0
                upd["input_type"]  = input_type
                upd["max_len"]     = int(max_len or 0)
                upd["required"]    = 1 if required else 0
                ...
                agol.add_features(fieldcfg_url, add)

        st.success("Domeinwaarden en instellingen opgeslagen.")
